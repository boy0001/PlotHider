        ...
        manager.addPacketListener(
            new PacketAdapter(main, ListenerPriority.NORMAL, PacketType.Play.Server.BLOCK_CHANGE) {
                public void onPacketSending(PacketEvent event) {
                    ...
                    if ((plot != null) && ((plot.isDenied(pp.getUUID())) || (
                        (!plot.isAdded(pp.getUUID())) && (Main.HIDE_FLAG.isTrue(plot))))) {
        ...
        manager.addPacketListener(new PacketAdapter(main, ListenerPriority.NORMAL,
            ...
            public void onPacketSending(PacketEvent event) {
                ...
                plot1 = (plot1 != null) && ((plot1.isDenied(pp.getUUID())) || (
                    (!plot1.isAdded(pp.getUUID())) && (Main.HIDE_FLAG.isTrue(plot1)))) ?
                    plot1 :
                    null;
                plot2 = (plot2 != null) && ((plot2.isDenied(pp.getUUID())) || (
                    (!plot2.isAdded(pp.getUUID())) && (Main.HIDE_FLAG.isTrue(plot2)))) ?
                    plot2 :
                    null;
                plot3 = (plot3 != null) && ((plot3.isDenied(pp.getUUID())) || (
                    (!plot3.isAdded(pp.getUUID())) && (Main.HIDE_FLAG.isTrue(plot3)))) ?
                    plot3 :
                    null;
                plot4 = (plot4 != null) && ((plot4.isDenied(pp.getUUID())) || (
                    (!plot4.isAdded(pp.getUUID())) && (Main.HIDE_FLAG.isTrue(plot4)))) ?
                    plot4 :
                    null;
                ...
                Plot denied =
                    plot3 != null ? plot3 : plot2 != null ? plot2 : plot1 != null ? plot1 : plot4;
        ...
        manager.addPacketListener(
            new PacketAdapter(main, ListenerPriority.NORMAL, PacketType.Play.Server.MAP_CHUNK) {
                public void onPacketSending(PacketEvent event) {
                    ...
                    Plot denied = plot3 != null ?
                        plot3 :
                        plot2 != null ? plot2 : plot1 != null ? plot1 : plot4;
                    PlotArea area = denied.getArea();
                    ...
                    try {
                        ...
                        for (layer = 0; layer < 16; layer++) {
                            if ((bitMask >> layer & 0x1) == 1) {
                                ...
                                int paletteLength = buffer.read();
                                buffer.skip(paletteLength);


                                while (buffer.read() != 2)
                                    ;

                                buffer.skip(bitsperBlock * 512);

                                buffer.skip(biomes ? 4096 : 2048);
